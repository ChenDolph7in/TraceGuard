...
from utils import average_weights, average_weights_ckks
...
class Structure(object):
    ...
    def __init__(self, args, encrypted_global_weights, global_weights, global_model, context=None, test_dataset=None):
        self.nn_architecture = global_model
        self.nn_weights = encrypted_global_weights
        self.nn_plain_weights = global_weights
        self.args = args
        self.system = self.hierarchy(self.nn_weights)
        self.local_system = self.hierarchy(self.nn_plain_weights)
        self.layers = len(args.mid_server) + 1
        self.context = context
        self.dataset = test_dataset

    # To do: when there is no middle layers
    def hierarchy(self, weights):
        """
        ...
        def build(layer_idx):
            ...
            if layer_idx == len(self.args.mid_server):
                ...
                layer_dict[0] = [server_set, weights]
                ...
                for server in range(num_server):
                    if server == (num_server - 1):
                        ...
                        user_set = set(np.random.choice(all_user, user_per_server,
                                                        replace=False))
                    layer_dict[server] = [user_set, weights]
            ...
            # other mid_layer
                ...
                for server in range(num_server):
                    ...
                    layer_dict[server] = [server_set, weights]
        ...
        for server_idx in first_layer_server.keys():
            if user_idx in first_layer_server[server_idx][0]:
                ...
                if download:
                    ...
                    encrypted_weights = first_layer_server[server_idx][1]
                    # Decrypt the weights
                    decrypted_weights = {}
                    for key in encrypted_weights.keys():
                        decrypted_values = encrypted_weights[key].decrypt()
                        decrypted_list = decrypted_values.tolist()
                        original_shape = decrypted_values.shape
                        decrypted_weights[key] = torch.tensor(decrypted_list, dtype=torch.float32).reshape(
                            original_shape)
                    model_weights = decrypted_weights
                else:
                    model_weights = copy.deepcopy(self.local_system[-1][user_idx])
                model.load_state_dict(model_weights)
                return model, server_idx

    # To do: how can we do model management? Do we need to test on all models?
    def upload_weights(self, local_weights, encrypted_weights, global_round):
        """
        ...
        def update(layer_idx):
            print(f"upload layer:{layer_idx}")
            if layer_idx == self.layers:
                return

            if layer_idx == 0:
                for server_idx in local_weights.keys():
                    # self.system[layer_idx][server_idx][1] = average_weights_ckks(
                    #     encrypted_weights[server_idx], context=self.context)
                    self.system[layer_idx][server_idx][1] = average_weights_ckks(
                        encrypted_weights[server_idx], context=self.context)
                    self.local_system[layer_idx][server_idx][1] = average_weights(
                        local_weights[server_idx])
                    print('| Global Round : {} |mid-server {}\'s user is uploading model weights to mid-server {}'
                          .format(global_round, server_idx, server_idx))
                ...
                for server_idx in self.system[layer_idx]:
                    ...
                    # self.system[layer_idx][server_idx][1] = average_weights_ckks(
                    #     prev_server_weights, context=self.context)
                    self.system[layer_idx][server_idx][1] = average_weights_ckks(
                        prev_server_weights, context=self.context)
                    print('| Global Round : {} |mid-server {}\'s user is uploading model weights to server {}'
                          .format(global_round, self.system[layer_idx][server_idx][0], server_idx))
    ...
    def model_management(self):
        ...
        def compare_accuracy(w1, w2):
            ...
            acc1, _, _, _ = test_inference(self.args, model, self.dataset)
            model.load_state_dict(w2)
            acc2, _, _, _ = test_inference(self.args, model, self.dataset)
