...
class Blockchain:
    ...
    def __init__(self, chain_dict=None):
        self.chain = self.from_json(chain_dict)
    ...
    def add_block_from_block(self, block):
        self.chain.append(block)
        return block
    ...
    def __add__(self, block):
        if isinstance(block, Block):
            ...
            # Perform custom addition logic for two instances of MyClass
            new_chain = [*self.chain, block]

            try:
                Blockchain.is_valid_chain(new_chain)
                new_blockchain = Blockchain()
                new_blockchain.chain = new_chain
                return new_blockchain
            except Exception as e:
                raise Exception(f'Cannot replace. The incoming block is invalid: {e}')
